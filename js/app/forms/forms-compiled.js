...
"./js/app/dataAccess/model-compiled.js", "./js/app/domain/carto-compiled.js"], function ( //coreConfig,
coreModel, carto) {
  ...
  function generateTabContainer(rootElement, className) {
    var element = $("#" + rootElement)[0];
    var containerHtml = "<div class='tab-pane clear-padding active' " + "id='" + className + "-Table'>";
  ...
  function createScheduler(elementName, data) {
    $("#" + elementName).kendoScheduler({
      date: Date.now(),
      views: ["day", "week", "timeline", "timelineWeek"],
      height: 600
    });
  ...
  function createTable(elementName, features, className, params) {
    ...
    var tableData = features.map(function (item) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      for (var field in row) {
        if (model.fields[field].type == "date") {
          ...
          data[i][field] = _d.getFullYear() + "/" + (_d.getMonth() + 1) + "/" + _d.getDate() + " " + _d.getHours() + ":" + _d.getMinutes() + ":" + _d.getMilliseconds();
        }
      }
    }

    utils.getColumnsState(className).then(function (doc) {
    ...
    function _prepareTable(elementName, className) {
      ...
      $("#" + elementName).kendoGrid({
        ...
        save: function (e) {
          ...
          db[className].getById(attributes["OBJECTID"]).then(function (data) {
        ...
        change: function (e) {
          ...
          db[className].getById(id).then(function (data) {
            var feature = data.features[0];
    ...
    $("#" + elementName).kendoGrid({
      ...
      change: (() => {
        var _ref = _asyncToGenerator(function* (e) {
          ...
          var action = yield db[className].getById(id);
  ...
  function _processField(_field, attributes, className, params) {
    ...
    switch (_field.type) {
      ...
      case "esriFieldTypeDate":
        ...
        result.data = correctDate.getFullYear() + "/" + (correctDate.getMonth() + 1) + "/" + correctDate.getDate() + " " + correctDate.getHours() + ":" + correctDate.getMinutes() + ":" + correctDate.getSeconds();
        break;
      /*
       case "file":
    ...
    if (checkDependencyParameters(_field.name) == true) {
      result.options.events = {
        change: function () {
          ...
          utils.getCascade(url, value, "name", dependency.parentKey, dependency.chieldKey).then(function (data) {
